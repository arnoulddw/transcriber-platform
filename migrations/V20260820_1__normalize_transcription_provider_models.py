...
def _normalize_user_api_keys(cursor) -> None:
    """Move legacy AssemblyAI provider-wide keys to the Universal model."""
    if not _table_exists(cursor, "user_api_keys"):
        return
    if not all(
        _column_exists(cursor, "user_api_keys", column)
        for column in ("id", "user_id", "provider_code", "model_slug")
    ):
        return

    # If a user already has a model-scoped Universal key, retain it and remove
    # duplicate provider-wide/legacy rows before changing the remaining rows.
    cursor.execute(
        """
        DELETE legacy
        ...
        INNER JOIN user_api_keys AS universal
           ...
           AND LOWER(TRIM(universal.provider_code)) = 'assemblyai'
           AND LOWER(TRIM(universal.model_slug)) = 'universal'
        WHERE LOWER(TRIM(legacy.provider_code)) = 'assemblyai'
          AND LOWER(TRIM(COALESCE(legacy.model_slug, ''))) IN ('', 'assemblyai')
    ...
    cursor.execute(
        ...
        INNER JOIN user_api_keys AS legacy_named
           ...
           AND LOWER(TRIM(legacy_named.provider_code)) = 'assemblyai'
           AND LOWER(TRIM(legacy_named.model_slug)) = 'assemblyai'
        WHERE LOWER(TRIM(legacy_blank.provider_code)) = 'assemblyai'
          AND LOWER(TRIM(COALESCE(legacy_blank.model_slug, ''))) = ''
    ...
    cursor.execute(
        ...
        WHERE LOWER(TRIM(provider_code)) = 'assemblyai'
          AND LOWER(TRIM(COALESCE(model_slug, ''))) IN ('', 'assemblyai')
        """
    )
...
def upgrade(db) -> None:
    ...
    try:
        ...
        _normalize_user_api_keys(cursor)
